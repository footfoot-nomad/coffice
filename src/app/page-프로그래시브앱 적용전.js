        ...
        <div ref={cardRef} className="shrink-0 flex flex-col items-center w-[100px] sm:w-[120px] md:w-[140px] border-2 border-black rounded-lg shadow-xs bg-white cursor-pointer overflow-hidden">
          ...
          {status?.status_user && (
            <div className="absolute right-1 top-1 z-10">
              <div className="badge bg-white shadow-md w-[36px] h-[20px] flex items-center justify-center">
                <span className="text-[9px] font-medium" style={{ color: getStatusColor(status.status_user) }}>
                  {status.status_user}
                </span>
              </div>
            </div>
...
export default function Home() {
  ...
  const createAttendanceEvent = async () => {
    ...
    try {
      ...
      if (distance > 50) { // 50미터 이상 떨어져 있을 경우
        setIsLoading(false); // 로딩 종료
        // DaisyUI 경고 알림 표시
        const alertDiv = document.createElement('div');
        alertDiv.className = 'alert alert-warning fixed top-4 left-1/2 transform -translate-x-1/2 z-50 w-[90%] max-w-[400px]';
        alertDiv.innerHTML = `
